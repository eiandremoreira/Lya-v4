...
class lyaDataBase {
    constructor(local) {
        ...
        if (!fs.existsSync(local)) fs.writeFileSync(local, '{"_lyaDataBaseVersion":2}');
        this.conteudo;
        this.local = local;
        try {
    ...
    get ping() {
        let inicio = Date.now()
        this.set('((((((PING))))))_internal_lyadb'+inicio, 0)
        this.delete('((((((PING))))))_internal_lyadb'+inicio)
